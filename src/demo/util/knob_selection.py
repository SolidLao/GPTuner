...
class KnobSelection(GPT):
    def __init__(self, api_base, api_key, db, dbms, candidate_knobs, workload_description, model, benchmark=None):
    ...
    def select_on_system_level(self):
        ...
        selected_knobs = {}
        for i in range(0, len(self.candidate_knobs), 50):
            success = False
            while not success:
                try:
                    candidates = self.candidate_knobs[i:i + 50]
                    ...
                    print(f"select_on_system_level ERROR: {e}, repeat...")
    ...
    def select_on_query_level(self):
        return dict()  ### disbaled for time sack
        ...
        for i, query in enumerate(query_list):
            ...
            for j in range(0, len(self.candidate_knobs), 50):
                candidates = self.candidate_knobs[i:i + 50]
                prompt = textwrap.dedent(f"""
    ...
    def select_interdependent_all_knobs(self):
        print(self.workload_description)
        # return ["shared_buffers"]
        print("selecting interdependent knobs...")
