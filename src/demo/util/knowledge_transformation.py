...
class KGTrans(GPT):
    def __init__(self, api_base, api_key, hardware_info=None, db="postgres", model=GPT.__init__.__defaults__[0]):
    ...
    def get_skill(self, knob, summary=None):
        if self.hardware_info == None:
            cpu_cores, ram_size, disk_size = self.get_hardware_info()
            disk_type = self.get_disk_type()
        else:
            cpu_cores, ram_size, disk_size, disk_type  = self.hardware_info
        if summary is None:
            try:
