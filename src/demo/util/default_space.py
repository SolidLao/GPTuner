...
class DefaultSpace:
    ...
    def __init__(self, dbms, test, timeout, structured_knowledge, suggested_values, target_knobs, workload_queries="", seed=1):
        ...
        self.target_knobs = self.knob_select(target_knobs)
        ...
        self.datas = self.get_knowledge_dict(structured_knowledge, suggested_values)
        self.log_file = f"./optimization_results/{self.dbms.name}/log/{self.seed}_log.txt"
        self.init_log_file()
        self.prev_end = 0

    def knob_select(self, candidate_knobs):
        target_knobs = []
    ...
    def get_knowledge_dict(self, structured_knowledge, suggested_values):
    ...
    def get_default_result(self):
        ...
        if self.test == "customized":   # customized workload, only support latency
            ...
            if flag:   # run successfully
                ...
                raise ValueError("Check your workload queries")
    ...
    def set_and_replay_ori(self, config, seed=0):
        ...
        dbms.reset_config()
        dbms.reconfigure()
        if self.workload_queries == "": # run benchbase
            # reload the data
            if self.test in self.benchmark_copy_db:
                print("Reloading the data")
                dbms._disconnect()
        ...
        else:    # customized workload, only support latency
            for knob in self.target_knobs:
                try:
                    control_para = config[f"control_{knob}"]
                    if control_para == "0":
            ...
            execution_time = end_ms - start_ms
            if flag:   # run successfully
                if execution_time > self.penalty:
