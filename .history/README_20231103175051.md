...
```
where `<dbms>` is the target DBMS (e.g., postgres or mysql), `<benchmark>` is the target workload (e.g., tpch or tpcc), `<timeout>` is the maximum time allowed to stress test the benchmark, `<seed>` is 
