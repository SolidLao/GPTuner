...
The optimization result is stored in `optimization_results/{dbms}/{stage}/{seed}/runhistory.json`, where {dbms} is the target DBMS, {stage} is `coarse` or `fine` and {seed} is the 
